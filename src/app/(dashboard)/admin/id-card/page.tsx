...
export default function AdminIdCardPage() {
  ...
  const downloadAsPng = async () => {
    ...
    try {
      ...
      const fileName = `${profile?.full_name?.replace(/ /g, '_')}_PrincipalID_${isFlipped ? 'Back' : 'Front'}.png`;
  ...
  const downloadAsPdf = async () => {
    ...
    try {
      ...
      await triggerPdfDownload(pdf, `${profile?.full_name?.replace(/ /g, '_')}_PrincipalID.pdf`);
    } catch (err) { console.error(err); }
    setIsDownloading(false);
  };
