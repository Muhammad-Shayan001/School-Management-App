...
export default function StudentIdCardPage() {
  ...
  const downloadAsPng = async () => {
    ...
    try {
      ...
      const fileName = `${profile?.full_name?.replace(/ /g, '_')}_StudentID_${isFlipped ? 'Back' : 'Front'}.png`;
  ...
  const downloadAsPdf = async () => {
    ...
    try {
      ...
      await triggerPdfDownload(pdf, `${profile?.full_name?.replace(/ /g, '_')}_StudentID.pdf`);
    } catch (err) { console.error(err); }
    setIsDownloading(false);
  };
