...
export default function TeacherStudentsPage() {
  ...
  async function handleApprove(userId: string) {
    ...
    setStudents((prev) =>
      prev.map((s) => (s.id === userId ? { ...s, status: 'approved' as UserStatus } : s))
    );
  ...
  async function handleReject(userId: string) {
    ...
    setStudents((prev) =>
      prev.map((s) => (s.id === userId ? { ...s, status: 'rejected' as UserStatus } : s))
    );
    setActionLoading(null);
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-text-secondary">Manage student registrations and access</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {students.map((student) => {
            const statusConfig = STATUS_CONFIG[student.status as UserStatus];
            return (
              <div key={student.id} className="glass-card p-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4 min-w-0">
                  <Avatar src={student.avatar_url} name={student.full_name || 'Student'} size="md" />
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-text-tertiary mt-0.5">{student.email}</p>
                    <p className="text-[10px] text-text-tertiary mt-0.5">Registered {formatDate(student.created_at)}</p>
                  </div>
                </div>
                {student.status === 'pending' && (
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <Button size="sm" onClick={() => handleApprove(student.id)} isLoading={actionLoading === student.id} leftIcon={<Check className="h-3.5 w-3.5" />}>Approve</Button>
                    <Button size="sm" variant="danger" onClick={() => handleReject(student.id)} isLoading={actionLoading === student.id} leftIcon={<X className="h-3.5 w-3.5" />}>Reject</Button>
                  </div>
                )}
              </div>
            );
